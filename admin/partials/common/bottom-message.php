...
if (!empty($this->managers) && in_array($user_ID, $this->managers) && !empty($work_fields[0]['date'])) : ?>
  <div class="support-hours-notices">
    ...
    <a class="button button-primary" href="<?php echo admin_url('admin.php?page=support-hours-settings'); ?>"><?php _e('Add new activity', 'support-hours'); ?></a>
    <?php if (count($work_fields) >= 5 && current_user_can('publish_pages') && 'index.php' == $pagenow) : ?>
      <a class="button button-secondary" href="<?php echo admin_url('admin.php?page=support-hours'); ?>"><?php _e('View all activities', 'support-hours'); ?></a>
    <?php endif; ?>
  ...
  if ('admin.php' == $pagenow) :
    ...
    <a class="button button-primary" href="<?php echo admin_url('admin.php?page=support-hours-settings'); ?>"><?php _e('Add first activity', 'support-hours'); ?></a>
  <?php
endif;
if ('index.php' == $pagenow) :
  ...
  if (100 == $percentage) {
    ...
    <a class="button button-primary" href="mailto:<?php echo $this->email; ?>?SUBJECT=<?php _e('Order Support Hours', 'support-hours'); ?> - <?php echo bloginfo('name'); ?>"><?php _e('Order Support Hours', 'support-hours'); ?></a>
  </div>
<?php endif; ?>